...
const notificationsAndEmailsFaqs = [
  {
    ...
    answer:
      "You'll receive alerts for logins, wallet activity, portfolio activity, approvals, operator changes, insights, and product updates.",
  },
  {
  ...
  },
  {
    ...
    answer:
      " Absolutely. Corporate admins receive email + in-app alerts for any operator-initiated transactions.",
  },
];
  ...
  return (
    ...
    >
      {notificationsAndEmailsFaqs.map((faq, index) => (
        <AccordionItem
          className={`${faq.question === notificationsAndEmailsFaqs[notificationsAndEmailsFaqs.length - 1].question ? "border-none" : ""}`}
          key={index}
          value={`item-${index + 1}`}
        >
