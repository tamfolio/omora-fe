...
const supportAndTroubleshooting = [
  {
    ...
    answer:
      "Immediately change your password and contact your Relationship Manager by selecting the Help Button on your dashboard",
  },
  {
    question: " My transaction is delayed. What should I do?",
    answer:
      "Check your transaction history for real-time updates. If it's beyond the estimated timeline, contact your Relationship Manager ",
  },
  {
    question: "How do I contact the Omora support team?",
    answer:
      "Your Relationship Manager is available 7 days a week. Alternatively, you can also leave an email to us on hello@omora.africa",
  },
];
  ...
  return (
    ...
    >
      {supportAndTroubleshooting.map((faq, index) => (
        <AccordionItem
          className={`${faq.question === supportAndTroubleshooting[supportAndTroubleshooting.length - 1].question ? "border-none" : ""}`}
          key={index}
          value={`item-${index + 1}`}
        >
