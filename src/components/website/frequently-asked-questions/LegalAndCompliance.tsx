...
const legalAndComplianceFaqs = [
  {
    ...
    answer:
      "Our Terms of Use, Privacy Policy, Cookies Policy, and Risk Disclosure are available in the footer of our site.",
  },
  {
    question: "Do you report my investments to tax authorities?",
    answer:
      "Currently, Omora does not file taxes on your behalf. Please consult a tax advisor regarding your crypto earnings.",
  },
];
  ...
  return (
    ...
    >
      {legalAndComplianceFaqs.map((faq, index) => (
        <AccordionItem
          className={`${faq.question === legalAndComplianceFaqs[legalAndComplianceFaqs.length - 1].question ? "border-none" : ""}`}
          key={index}
          value={`item-${index + 1}`}
        >
