...
function Questionnaire({ onClose, onSubmit }: QuestionnaireProps) {
  ...
  const handleSubmit = () => {
    if (onSubmit) {
      console.log('📤 Submitting answer IDs:', selectedAnswerIds);
      onSubmit(selectedAnswerIds);
    }
  };

  const completedQuestions = Object.keys(selectedAnswerIds).length;
  const canSubmit = completedQuestions === questions.length && questions.length > 0;
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-2xl p-8 max-w-md">
          <div className="text-center">
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-2">Failed to Load Questions</h2>
      ...
      <div className="relative bg-white rounded-2xl w-full max-w-2xl shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="px-8 py-4 border-b border-gray-100">
          <p className="text-sm text-gray-600">
            You have finished <span className="text-cyan-600 font-medium">{completedQuestions}/{questions.length}</span> questions.
          </p>
        </div>
