...
export default function PersonalInformation({
  ...
  useEffect(() => {
    ...
    if (Array.isArray(verification)) {
      const bvnRecord = verification.find(
        (v: any) => v.type === "BVN" && v.status === "C",
      );
      const ninRecord = verification.find(
        (v: any) => v.type === "NIN" && v.status === "C",
      );
      if (bvnRecord) {
  ...
  const handleVerifyBVN = async () => {
    ...
    try {
      const response = await fetch("/api/user/verification/verify/bvn", {
        method: "POST",
  ...
  const handleVerifyNIN = async () => {
    ...
    try {
      const response = await fetch("/api/user/verification/verify/nin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        "/api/user/api/v1/onboarding/personal/information",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          throw new Error(
            "Onboarding endpoint not found (404). Please verify the proxy path.",
          );
        }
        throw new Error(
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-xl text-sm bg-gray-50"
              disabled
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-xl text-sm focus:ring-teal-500"
              placeholder="Racheal"
            />
          </div>
          ...
          <div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                {bvnStatus === "verifying" && (
                  <div className="absolute right-3 top-2.5 w-4 h-4 border-2 border-teal-500 border-t-transparent rounded-full animate-spin"></div>
                )}
                {bvnStatus === "success" && (
            ...
            <input
              ...
              className={`w-full px-3 py-2 border border-gray-300 rounded-xl text-sm ${
                bvnStatus === "success" || ninStatus === "success"
                  ? "bg-gray-50"
                  : ""
              }`}
          ...
          <button
            type="submit"
            disabled={
              isSubmitting || bvnStatus !== "success" || ninStatus !== "success"
            }
            className="w-full bg-teal-500 hover:bg-teal-600 text-white py-3 rounded-xl font-medium text-sm disabled:opacity-50"
          >
