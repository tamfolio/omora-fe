  ...
  const handleBackToBalance = () => {
    ...
    const currentBalances = JSON.parse(
      localStorage.getItem("userBalances") ||
        '{"usdc": 0, "naira": 0, "usdt": 0}',
    );

    // Update balances based on conversion
    if (conversionData.fromCurrency === "NGN") {
      currentBalances.naira = Math.max(
        0,
        currentBalances.naira - parseFloat(conversionData.fromAmount),
      );
      ...
      currentBalances.usdt = Math.max(
        0,
        currentBalances.usdt - parseFloat(conversionData.fromAmount),
      );
      ...
      currentBalances.usdc = Math.max(
        0,
        currentBalances.usdc - parseFloat(conversionData.fromAmount),
      );
      if (conversionData.toCurrency === "USDT") {
        currentBalances.usdt += parseFloat(conversionData.toAmount);
      }
      ...
      <div className="bg-white rounded-2xl p-8 w-full max-w-md mx-4 shadow-xl">
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-xl font-semibold text-gray-900 mb-2">
            Funds Converted Successfully
          </h2>

          {/* Success Message */}
          <p className="text-sm text-gray-600 mb-1">
            You have successfully converted {conversionData.fromCurrency}{" "}
            {parseFloat(conversionData.fromAmount).toLocaleString()} to{" "}
