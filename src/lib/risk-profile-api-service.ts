...
const riskProfileApiService = {
  ...
  saveRiskProfile: async (profileName: string) => {
    try {
      ...
      if (!profileId) {
        console.warn(`Unknown profile name: ${profileName}, defaulting to Balanced (5)`);
      }
      ...
      console.log(`💾 Saving Profile ID ${payload.riskProfile} for ${profileName}`);

      // Points to your User Service via Proxy
      const response = await axios.post('/api/proxy/user/api/v1/risk-profile', payload);
      return response.data;
    } catch (error) {
  ...
  mapProfileToUserFriendly: (apiProfile: string): 'conservative' | 'balanced' | 'aggressive' => {
    const normalize = apiProfile?.toLowerCase() || '';
    if (normalize.includes('conservative') || normalize.includes('low')) return 'conservative';
    if (normalize.includes('aggressive') || normalize.includes('high')) return 'aggressive';
    return 'balanced';
  }
};
